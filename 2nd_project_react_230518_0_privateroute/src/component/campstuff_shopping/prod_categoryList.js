...
const ProdCategory = ({ cartCal }) => {
  const dispatch = useDispatch();
  const navigator = useNavigate();
  const { currentPage, category } = useParams();
  ...
  const getProdList = (currentPage, category) => {
    dispatch(prodActions.getCategoryList(currentPage, category));
    navigator(`/prod/list/category/${currentPage}/${category}`);
  };
  ...
  useEffect(() => {
    getProdList(currentPage, category);
  }, []); ///////////////////
