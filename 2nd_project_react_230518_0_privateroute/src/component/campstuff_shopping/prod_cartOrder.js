...
const CartProdOrder = () => {
  ...
  const prodList = useSelector((state) => state.prod.orderCartList);
  ...
  const totalPrice = () => {
    let price = 0;
    prodList.map((prod) => {
      price = price + Number(prod.prodPrice) * prod.prodCartCount;
    });
    return price;
  };
  ...
  useEffect(() => {
    ...
    // dispatch(prodActions.getUserInfo());

    //체크된 ProdList 가져오기
    // dispatch(prodActions.getCartOrderList(localStorage));
    dispatch(
      prodActions.getCartOrderList(localStorage.getItem("userKeynum"), {
        headers: {
          Authorization: localStorage.getItem("Authorization"),
        },
      })
  ...
  useEffect(
    (e) => {
      ...
      //   console.log(inputs.deliveryMemo);
      // 토스 관련
    ...
    [inputs, storemethod, memberDatil, localStorage]
  );
    ...
    <form>
      <div className={style.prodPay}>
          ...
          <table className={style.payListTable}>
              ...
              {prodList.map((prod) => {
                return (
                  <tr>
                    ...
                    <td>
                      {(
                        Number(prod.prodPrice) * prod.prodCartCount
                      ).toLocaleString("kp-KR")}
                    </td>
                  </tr>
                );
              })}
            ...
            <tfoot className={style.prodPayTotal}>
              <tr>
                <td>배송비 0원</td>
                <td colspan='2'>
                  총 결제금액 {totalPrice().toLocaleString("kp-KR")}원
                </td>
              </tr>
            </tfoot>
          </table>
        ...
        <div className={style.deliveryInfo}>
          <p className='middelText'>배송 정보</p>
          <hr />
